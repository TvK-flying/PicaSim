...
if [ -f "$SOURCE_DIR/VERSIONS.txt" ]; then
    VERSION=$(grep -oE 'Version [0-9.]+' "$SOURCE_DIR/VERSIONS.txt" | head -1 | sed 's/Version //' || echo "1.0.0")
fi
...
# Copy app icon (pre-generated by resources/generate_icons.py)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ICNS_FILE="$SCRIPT_DIR/resources/PicaSim.icns"
if [ -f "$ICNS_FILE" ]; then
    echo "Copying app icon..."
    cp "$ICNS_FILE" "$RESOURCES_DIR/"
else
    echo "Warning: PicaSim.icns not found. Run 'python3 resources/generate_icons.py' to generate it."
fi
